...
const FIREBASE_TOOLS = [
    {
        name: "extractIntent",
        description: "Agent 1 — Intent Extractor. Parses natural language input (Urdu, Roman Urdu, English) into structured service request fields: service type, location, time, urgency, confidence score, and detected language.",
        inputSchema: {
            type: "object",
            properties: {
                userInput: {
    ...
    },
    {
        ...
        description: "Agent 2 — Provider Discovery. Searches Firestore for approved providers matching the service category within the given radius of the customer's coordinates. Returns providers with distanceKm calculated.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "rankProviders",
        description: "Agent 3 — Ranking Engine. Scores and ranks discovered providers using weighted criteria: distance (40%), rating (30%), on-time score (20%), risk score (10%). Returns ranked list with score breakdowns and reasoning.",
        inputSchema: {
            type: "object",
            properties: {
                providers: {
    ...
    },
    {
        ...
        description: "Agent 4 — Pricing Engine. Calculates itemized pricing breakdown (base fee, travel fee, urgent surcharge, platform fee, total) in PKR for a specific provider and distance.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "selectMatch",
        description: "Agent 5 — Matchmaker. Selects the best provider from the ranked list considering pricing, and generates a human-readable reasoning summary for the customer.",
        inputSchema: {
            type: "object",
            properties: {
                rankedProviders: {
    ...
    {
        name: "lockBooking",
        description: "Agent 6 — Lock & Book. Simulates booking confirmation by writing a booking record to Firestore with status 'booked'. Generates a BK-XXXX booking ID.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "followUp",
        description: "Agent 7 — Follow-Up Automation. Sends simulated follow-up messages (reminder, en_route alert, completion prompt) for a booking.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "review",
        description: "Agent 8 — Review & Badges. Processes a customer review, updates provider rating and badges in Firestore.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "dispute",
        description: "Agent 9 — Dispute Resolution. Automatically resolves customer disputes (overcharge, no-show, cancellation, poor quality) with refund calculations and provider penalties.",
        inputSchema: {
            type: "object",
            properties: {
                bookingId: { type: "string", description: "Booking ID" },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        const response = await axios.post(url, { data: toolArgs }, {
